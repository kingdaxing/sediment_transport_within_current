...
function F = q_SixEqn_lower(unknowns, constants)
    % unknown variables
    V = unknowns(1);
    D = unknowns(2);
    D_prime = unknowns(3);
    ...
    F= [q - V*D;
        V - Uf_prime *(6+2.5*log(D_prime/k_N)) ;
        Uf_prime - sqrt(g*D_prime*Slope);
        D - D_prime*(theta/theta_prime);
        theta - D * Slope/(s-1)/d;
        theta_prime - (0.06 + 0.3*theta^1.5) % for theta_prime <= 0.55, lower regime
                ]; 
end